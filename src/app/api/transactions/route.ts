...
import { calculateStockDeduction, getEffectiveStock, convertFromBaseUnit, calculateQuantityFromPrice } from '@/lib/product-units'
        ...
        if (itemIds.length > 0) {
          ...
          for (const item of transaction.items) {
            ...
            if (rawItem) {
              ...
              console.log(`[DEBUG GET] Updated quantity from raw SQL - itemId: ${item.id}, Prisma qty: ${item.quantity}, Raw SQL qty: ${rawQty}`)
            }
          }
        }
      }
      ...
      return {
        ...
        items: transaction.items.map((item: any) => {
          ...
          if (item.quantity !== null && item.quantity !== undefined) {
            ...
            else if (item.quantity && typeof item.quantity === 'object' && 'valueOf' in item.quantity) {
          ...
          console.log(`[DEBUG GET] Item quantity - raw: ${JSON.stringify(item.quantity)}, type: ${typeof item.quantity}, constructor: ${item.quantity?.constructor?.name}, hasToNumber: ${typeof item.quantity?.toNumber === 'function'}, converted: ${quantity}`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of items) {
      ...
      if (item.sellingUnitId) {
        ...
        if (item.priceBasedAmount && sellingUnit.allowPriceBased) {
          ...
          if (priceAmount > 0) {
            quantityInBaseUnit = calculateQuantityFromPrice(priceAmount, sellingUnit)
            console.log(`[DEBUG] Price-based: priceAmount=${priceAmount}, quantityInBaseUnit=${quantityInBaseUnit}, sellingPrice=${sellingUnit.sellingPrice}, conversionFactor=${sellingUnit.conversionFactor}`)
          } else {
            return NextResponse.json(
          ...
          console.log(`[DEBUG] Normal: item.quantity=${item.quantity}, qty=${qty}, quantityInBaseUnit=${quantityInBaseUnit}`)
        }
      } else {
      ...
      console.log(`[DEBUG] TransactionItem prepared - quantity: ${quantityInBaseUnit}, price: ${itemPrice}, subtotal: ${subtotal}`)
    }
    ...
    console.log(`[DEBUG] Creating ${transactionItems.length} transaction items...`)
    for (const item of transactionItems) {
      ...
      console.log(`[DEBUG] Creating item - qtyValue: ${qtyValue}, priceValue: ${priceValue}, subtotalValue: ${subtotalValue}`)
      
      // Validate quantity
      if (isNaN(qtyValue) || qtyValue <= 0) {
        console.error(`[ERROR] Invalid quantity: ${qtyValue} for product ${item.productId}`)
        return NextResponse.json(
      ...
      console.log(`[DEBUG] Using raw SQL insert - ID: ${itemId}, qtyValue: ${qtyValue} (type: ${typeof qtyValue}), priceValue: ${priceValue}, subtotalValue: ${subtotalValue}`)
      ...
      console.log(`[DEBUG] String values - qty: "${qtyStr}", price: "${priceStr}", subtotal: "${subtotalStr}"`)
      ...
      console.log(`[DEBUG] Item inserted with raw SQL - ID: ${itemId}, quantity: ${qtyStr}`)
      ...
      if (verifyResult && verifyResult.length > 0) {
        ...
        console.log(`[DEBUG] Raw SQL verification - quantity: ${stored.quantity}, type: ${stored.qty_type}`)
      }
      ...
      if (verifyItem) {
        const verifyQty = verifyItem.quantity instanceof Prisma.Decimal 
          ? verifyItem.quantity.toNumber() 
          : Number(verifyItem.quantity)
        console.log(`[DEBUG] Prisma verification - quantity: ${verifyQty} (type: ${typeof verifyItem.quantity}), expected: ${qtyValue}`)
        if (Math.abs(verifyQty - qtyValue) > 0.0001) {
          console.error(`[ERROR] Quantity mismatch! Expected: ${qtyValue}, Stored: ${verifyQty}, Diff: ${Math.abs(verifyQty - qtyValue)}`)
        }
      } else {
        console.error(`[ERROR] Failed to fetch inserted item with ID: ${itemId}`)
      }
    }
    ...
    if (transactionWithItems.items && transactionWithItems.items.length > 0) {
      transactionWithItems.items.forEach((item: any, idx: number) => {
        console.log(`[DEBUG] Item ${idx} from DB: quantity=${item.quantity?.toString() || item.quantity}, type=${typeof item.quantity}`)
      })
    }

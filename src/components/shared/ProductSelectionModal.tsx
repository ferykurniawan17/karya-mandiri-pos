...
interface ProductSelectionModalProps {
  ...
  // Pre-loaded data (optional - if not provided, will fetch internally)
  ...
  // Custom API endpoint (optional - defaults to /api/products)
  productsApiEndpoint?: string;
  
  // Custom product render function (optional)
  renderProductCard?: (product: Product, onClick: () => void) => React.ReactNode;
  
  // Custom empty state render (optional)
  renderEmptyState?: () => React.ReactNode;
  
  // Additional query parameters for product fetch
...
export default function ProductSelectionModal({
  ...
  excludeProductIds = [],
  title = "Pilih Produk",
  description = "Pilih produk dari daftar",
  searchPlaceholder = "Cari berdasarkan nama, SKU, kategori, brand...",
  emptyMessage = "Belum ada produk",
  emptyFilterMessage = "Tidak ada produk yang sesuai dengan filter",
  ...
  showTagFilter = true,
  productsApiEndpoint = "/api/products",
  renderProductCard,
  renderEmptyState,
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      let url = `${productsApiEndpoint}?search=${encodeURIComponent(search)}`;
  ...
  const defaultRenderProductCard = (product: Product, onClick: () => void) => (
    <button
      key={product.id}
      type="button"
      onClick={onClick}
  ...
  const defaultRenderEmptyState = () => (
    <div className="text-center py-12 text-gray-500">
      {search || categoryFilter !== "all" || selectedTagIds.length > 0 || selectedBrandIds.length > 0
        ? emptyFilterMessage
        : emptyMessage}
    </div>
  );

  const hasActiveFilters = search || categoryFilter !== "all" || selectedTagIds.length > 0 || selectedBrandIds.length > 0;
      ...
      <DialogContent className="max-w-6xl h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0">
          ...
          <div className="space-y-4 pb-4 border-b flex-shrink-0">
            ...
            {(showCategoryFilter || showBrandFilter || showTagFilter) && (
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-2">
                {products.map((product) =>
                  renderProductCard ? (
                    renderProductCard(product, () => handleProductClick(product))
                  ) : (
                    defaultRenderProductCard(product, () => handleProductClick(product))
                  )
                )}
              </div>
            )}
